...
public class AppointmentController {
    ...
    private void addAppointmentToTable(Appointment appointment) {
        ...
        model.addRow(new Object[]{
                ...
                appointment.getClinicianId(), appointment.getAppointmentDate(),
                appointment.getAppointmentTime(), appointment.getAppointmentType(),
                appointment.getStatus(), appointment.getReasonForVisit()
        });
    }
    ...
    private void addAppointment() {
        String[] labels = {"Patient ID*", "Clinician ID*", "Date (DD/MM/YYYY)*", "Time*"};
        String[] names = {"patientId", "clinicianId", "date", "time"};

        JFrame parent = (JFrame) SwingUtilities.getWindowAncestor(view);
        GenericAddEditDialog dialog = new GenericAddEditDialog(parent, "Add New Appointment", null, labels, names, Appointment.class);
        ...
        if (!values.isEmpty() && values.get("patientId") != null && !values.get("patientId").isEmpty()) {
            String newId = "A" + String.format("%03d", appointments.size() + 1);
            Appointment newAppointment = new Appointment(newId, values.get("patientId"),
                    values.get("clinicianId"), "F001", values.get("date"), values.get("time"),
                    "30", "Follow-up", "Scheduled", "Routine checkup", "None",
                    "21/12/2025", "21/12/2025");

            appointments.add(newAppointment);
            addAppointmentToTable(newAppointment);
            saveAppointments();
            JOptionPane.showMessageDialog(view, "Appointment " + newId + " added successfully!");
        }
    }
    ...
    private void editAppointment() {
        ...
        String[] labels = {"Patient ID", "Date", "Time"};
        String[] names = {"patientId", "date", "time"};

        JFrame parent = (JFrame) SwingUtilities.getWindowAncestor(view);
        GenericAddEditDialog dialog = new GenericAddEditDialog(parent, "Edit Appointment", appointments.get(selectedRow), labels, names, Appointment.class);
        dialog.setVisible(true);

        Map<String, String> values = dialog.getFieldValues();
        if (!values.isEmpty() && values.get("patientId") != null && !values.get("patientId").isEmpty()) {
            Appointment appointment = appointments.get(selectedRow);
            appointment.setPatientId(values.get("patientId"));
            appointment.setAppointmentDate(values.get("date"));
            appointment.setAppointmentTime(values.get("time"));
            refreshTable();
            saveAppointments();
    ...
    private void deleteAppointment() {
        int selectedRow = view.getAppointmentTable().getSelectedRow();
        if (selectedRow == -1) {
            JOptionPane.showMessageDialog(view, "Please select an appointment to delete");
            return;
        }
        int confirm = JOptionPane.showConfirmDialog(view, "Are you sure you want to delete this appointment?", "Confirm Delete", JOptionPane.YES_NO_OPTION);
    ...
    private void saveAppointments() {
        ...
        data.add(new String[]{"appointment_id","patient_id","clinician_id","facility_id","appointment_date","appointment_time","duration_minutes","appointment_type","status","reason_for_visit","notes","created_date","last_modified"});
        for (Appointment a : appointments) {
            data.add(new String[]{a.getAppointmentId(),a.getPatientId(),a.getClinicianId(),a.getFacilityId(),a.getAppointmentDate(),a.getAppointmentTime(),a.getDurationMinutes(),a.getAppointmentType(),a.getStatus(),a.getReasonForVisit(),a.getNotes(),a.getCreatedDate(),a.getLastModified()});
        }
